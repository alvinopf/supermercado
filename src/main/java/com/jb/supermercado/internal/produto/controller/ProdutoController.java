...
public class ProdutoController {
    ...
    @PostMapping
    public ResponseEntity<Void> cadastrar(@Valid @RequestBody ProdutoRequestRecord request) {
        this.produtoService.cadastrarProduto(request);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }

    @PutMapping("/{id}")
    public ResponseEntity<Void> atualizar(@PathVariable Long id, @Valid @RequestBody ProdutoRequestRecord request) {
        this.produtoService.atualizarProduto(id, request);
